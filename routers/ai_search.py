...
def understand_query_node(state: SearchState) -> SearchState:
    ...
    response = get_llm(0.7).invoke([HumanMessage(content=understand_prompt)])
...
def tavily_search_node(state: SearchState) -> SearchState:
    ...
    try:
        ...
        return {
            ...
            "step": "searched",
            "messages": [AIMessage(content="✅ Search complete! Relevant information found; compiling the answer for you...")]
        }
    except Exception as e:
...
# traffic_search_node
def traffic_search_node():
    print("🚗 Bayern traffic detected, fetching BR.de data...")
    with httpx.Client(
...
def generate_answer_node(state: SearchState) -> SearchState:
    ...
    if state["step"] == "search_failed":
        ...
        response = get_llm(0.7).invoke([HumanMessage(content=fallback_prompt)])
...
async def ai_search(payload: dict):
    ...
    try:
        ...
        print("\n" + "="*60 + "\n")
        return {"search": "No response generated."}
